...
class ScMoGCNWrapper(object):
    ...
    def evaluate(self, g, idx, labels):
        self.model.eval()
        with torch.no_grad():
    ...
    def fit(self, g, y, split=None, eval=True, verbose=2, y_test=None, logger=None):
        ...
        for epoch in range(kwargs['epoch']):
            ...
            val.append(self.evaluate(g, split['valid'], y[split['valid']]))
            if verbose > 1:
            ...
            if eval:
                te.append(self.evaluate(g, np.arange(TRAIN_SIZE, CELL_SIZE), y_test))
                if verbose > 1:
    ...
    def saint(self, g, y, split=None, eval=True, verbose=2, y_test=None, logger=None):
        kwargs = vars(self.args)
        PREFIX = kwargs['prefix']
        CELL_SIZE = kwargs['CELL_SIZE']
        TRAIN_SIZE = kwargs['TRAIN_SIZE']
        ...
        sampler = dgl.dataloading.NeighborSampler([{('feature', 'pathway', 'feature'):0,
                                                    ('cell', 'occur', 'feature'):100,
                                                    ('feature', 'entail', 'cell'):100},
        ...
        for epoch in range(kwargs['epoch']):
            ...
            val.append(self.evaluate(g_origin, split['valid'], y[split['valid']]))
            ...
            if eval:
                te.append(self.evaluate(g_origin, np.arange(TRAIN_SIZE, CELL_SIZE), y_test))
                if verbose > 1:
                    logger.write(f'testing loss:  {te[-1]}\n')
                    logger.flush()
